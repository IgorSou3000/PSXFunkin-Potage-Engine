...
void Week6_Freaks_Draw(Back_Week6 *this, fixed_t x, fixed_t y, boolean flip)
{
	...
	fixed_t ox = x - ((fixed_t)cframe->off[0] * FIXED_UNIT);
	fixed_t oy = y - ((fixed_t)cframe->off[1] * FIXED_UNIT);
	
	RECT src = {cframe->src[0], cframe->src[1], cframe->src[2], cframe->src[3]};
	RECT_FIXED dst = {ox, oy, src.w * FIXED_UNIT, src.h * FIXED_UNIT};
...
void Back_Week6_DrawBG(StageBack *back)
{
	...
	Week6_Freaks_Draw(this, FIXED_DEC(-110,1) - fx, FIXED_DEC(38,1) - fy, false);
	Week6_Freaks_Draw(this,  FIXED_DEC(-20,1) - fx, FIXED_DEC(38,1) - fy, true);
	Week6_Freaks_Draw(this,   FIXED_DEC(70,1) - fx, FIXED_DEC(38,1) - fy, false);
	...
	for (size_t i = 0; i < COUNT_OF(fg_tree); i++, fg_tree_p++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			treep[j].x += ((MUtil_Cos(FIXED_MUL(animf_count, fg_tree_p->off[j])) * FIXED_DEC(3,1)) / 256) - fx;
			treep[j].y += ((MUtil_Sin(FIXED_MUL(animf_count, fg_tree_p->off[j])) * FIXED_DEC(3,1)) / 256) - fy;
		}
	...
	Stage_DrawTex(&this->tex_back0, &street_src, &street_dst, stage.camera.bzoom);
	
	//Draw sky
	fx = stage.camera.x / 10;
	fy = stage.camera.y / 10;
