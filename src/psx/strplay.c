...
static void strDoPlayback(char** filedir, int xres, int yres, int framecount);
...
int PlayStr(int xres, int yres, int xpos, int ypos, const char * filedir, int framecount) {
	...
	*/
	...
	strPlayDone=0;
	strDoPlayback((char**)filedir, xres, yres, framecount);
...
static void strDoPlayback(char** filedir, int xres, int yres, int framecount) {
	...
	*/
	...
	if (CdSearchFile(&file, (char*)filedir) == 0) {
...
static u_long *strNext(STRENV *strEnv) {
	...
	*/
	...
	if (sector->frameCount >= (u_long)strNumFrames)
		strPlayDone = 1;
